...
const Home = () => {
  ...
  const handlePrevious = () => {
    setCurrentIndex((index) => (index - 1 + slidingImages.length) % slidingImages.length);
  };
    ...
    <div className='h-screen'>
      <div className='h-3/5'>
        <div className='h-[90%] bg-red-400 m-2 flex items-center justify-center rounded-lg'>
          <img src={slidingImages[currentIndex]} alt={`Slide ${currentIndex + 1}`} className='w-full h-full object-cover rounded-lg' />
        </div>
        <div className='w-full flex justify-evenly items-center mt-8'>
          <button onClick={handlePrevious} className='border-2 w-[30%] border-orange-400 rounded-lg font-semibold'>Previous</button>
          <button onClick={handleNext} className='w-[30%] bg-orange-400 rounded-lg text-white font-semibold'>Next</button>
        </div>
      </div>
      <div className='h-2/5 flex flex-col gap-5 items-center p-5 mt-20'>
        <Link to='/signin' className='border-2 border-orange-200 p-2 w-full rounded-lg text-lg font-semibold text-center'>Sign In</Link>
        <Link to='/signup' className='bg-orange-400 text-white p-2 w-full rounded-lg text-lg font-semibold text-center'>Sign Up</Link>
      </div>
    </div>
  );
};
